...
def test_storage_credential():
    ...
    try:
        ...
        auth_mode = (body.get("auth_mode") or "pat").strip().lower()
        ...
        if auth_mode not in ("pat", "service_principal"):
            return jsonify({"success": False, "error": "auth_mode must be 'pat' or 'service_principal'"})

        if auth_mode == "service_principal":
            tenant_id = (body.get("azure_tenant_id") or cfg.get("azure_tenant_id") or "").strip()
            client_id = (body.get("azure_client_id") or cfg.get("azure_client_id") or "").strip()
            client_secret = (body.get("azure_client_secret") or cfg.get("azure_client_secret") or "").strip()
            missing = [
            ...
            if missing:
                return jsonify({"success": False, "error": "Missing required fields for Service Principal mode: " + ", ".join(missing)})
            ...
            ).get_token(dbx_scope).token
            headers = {"Authorization": f"Bearer {aad_token}"}
        else:
            if not token or is_masked(token):
                token = get_databricks_token()
            if not token:
                return jsonify({"success": False, "error": "PAT token is required for PAT auth mode"})
            headers = {"Authorization": f"Bearer {token}"}
        ...
        if get_resp.status_code != 200:
            return jsonify({
                "success": False,
                "error": f"Failed to look up credential (HTTP {get_resp.status_code}): {get_resp.text[:200]}",
                "auth_mode": auth_mode,
            })
        cred = get_resp.json()
        ...
        # 3) Validate the credential can actually reach storage.
        ...
        return jsonify({
            "success": False,
            "error": "Storage access validation failed. If connector/permissions were just updated, wait 5-10 minutes and retry.",
            "detail": err_msg[:300],
            "credential_id": credential_id,
            "owner": owner,
            "access_connector_id": access_connector_id,
